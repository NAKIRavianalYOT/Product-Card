...
---
1. [*] Создать папку "async", в ней async.html, async.js. Дополнительно создать JSON файл и заполнить его данными: в нем будет массив с пользователями (users), состоящий из объектов с полями: id, name, surname, email, age (некий симулятор базы данных). Упомянутые поля — обязательны, остальные на ваш вкус.
